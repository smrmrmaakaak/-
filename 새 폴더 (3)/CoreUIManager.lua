...
local mainUIPopupFrameNames = { -- 주요 팝업으로 간주될 프레임 이름 목록
	...
	-- "ConsumableItemListFrame" -- <<< 이 줄이 있다면 제거 또는 주석 처리합니다.
}
-- ##############################################################
...
function CoreUIManager.ShowFrame(frameName, show)
	...
	-- ConsumableItemListFrame은 CompanionFrame 내부에 있으므로, OpenMainUIPopup에서 처리
...
function CoreUIManager.OpenMainUIPopup(frameNameToOpen)
	...
	-- ConsumableItemListFrame은 mainUIPopupFrameNames에 없어야 하므로, isFrameLegitMainPopup은 false가 됨
	-- 따라서 이 프레임은 OpenMainUIPopup으로 열릴 때 다른 창을 닫지 않음. (ShowFrame을 통해 직접 Visible 제어)
	...
	-- ConsumableItemListFrame이 아닐 경우에만 다른 메인 팝업 닫기 로직 수행
	if isFrameLegitMainPopup and currentActiveMainUIPopupFrame and currentActiveMainUIPopupFrame.Name ~= frameNameToOpen then
		...
		local managerName = currentActiveMainUIPopupFrame.Name:gsub("Frame", "UIManager")
		-- ConsumableItemListFrame은 여기서 닫힐 일이 없도록 위에서 처리
		local manager = ModuleManager:GetModule(managerName)
		local showFunctionName = "Show" .. currentActiveMainUIPopupFrame.Name:gsub("Frame", "")

		if manager and manager[showFunctionName] then
			print("CoreUIManager.OpenMainUIPopup: Calling " .. managerName .. "." .. showFunctionName .. "(false) for cleanup.")
			local success, err = pcall(manager[showFunctionName], manager, false)
			if not success then warn("Error during cleanup call to", managerName, ".", showFunctionName, ":", err) end
		else
			currentActiveMainUIPopupFrame.Visible = false
			print("CoreUIManager.OpenMainUIPopup: No specific manager/show function for "..currentActiveMainUIPopupFrame.Name..", directly hiding.")
		end
		currentActiveMainUIPopupFrame = nil -- 명시적으로 nil 처리
	end

	frameToOpenInstance.Visible = true
	if isFrameLegitMainPopup then -- 메인 팝업일 경우에만 currentActiveMainUIPopupFrame 업데이트
...
function CoreUIManager.CloseAllMainUIPopups()
	...
	if currentActiveMainUIPopupFrame and currentActiveMainUIPopupFrame.Visible then
		...
		local managerName = currentActiveMainUIPopupFrame.Name:gsub("Frame", "UIManager")
		if currentActiveMainUIPopupFrame.Name == "ConsumableItemListFrame" then 
			-- ConsumableItemListFrame은 OpenMainUIPopup으로 직접 닫히지 않으므로,
			-- 만약 이 함수가 호출될 때 열려있다면 CompanionUIManager를 통해 닫아야 함.
			managerName = "CompanionUIManager"
		end
		local manager = ModuleManager:GetModule(managerName)
		local showFunctionName = ""
		if currentActiveMainUIPopupFrame.Name == "ConsumableItemListFrame" then
			showFunctionName = "ShowConsumableItemList"
		else
			showFunctionName = "Show" .. currentActiveMainUIPopupFrame.Name:gsub("Frame", "")
		end

		if manager and manager[showFunctionName] then
			print("CoreUIManager.CloseAllMainUIPopups: Calling " .. managerName .. "." .. showFunctionName .. "(false) for cleanup.")
			local success, err = pcall(manager[showFunctionName], manager, false)
			if not success then warn("Error during cleanup call in CloseAllMainUIPopups to", managerName, ".", showFunctionName, ":", err) end
		else
			currentActiveMainUIPopupFrame.Visible = false
			print("CoreUIManager.CloseAllMainUIPopups: No specific manager/show function for "..currentActiveMainUIPopupFrame.Name..", directly hiding.")
		end
		currentActiveMainUIPopupFrame = nil
	end
	-- 추가적으로 ConsumableItemListFrame도 닫아주기 (혹시 모르니)
	local _, bgFrame, _ = getCoreUIElements()
	if bgFrame then
		local compFrame = bgFrame:FindFirstChild("CompanionFrame")
		if compFrame then
			local consFrame = compFrame:FindFirstChild("ConsumableItemListFrame")
			if consFrame and consFrame.Visible then
				local compUIMgr = ModuleManager:GetModule("CompanionUIManager")
				if compUIMgr and compUIMgr.ShowConsumableItemList then
					pcall(compUIMgr.ShowConsumableItemList, compUIMgr, false)
					print("CoreUIManager.CloseAllMainUIPopups: Explicitly closed ConsumableItemListFrame.")
				else
					consFrame.Visible = false
				end
			end
		end
	end
...
function CoreUIManager.SwitchFrame(frameName) 
	...
	if isMainPopup then
		...
		if currentActiveMainUIPopupFrame and currentActiveMainUIPopupFrame.Name ~= frameName then
			currentActiveMainUIPopupFrame = nil
		end
