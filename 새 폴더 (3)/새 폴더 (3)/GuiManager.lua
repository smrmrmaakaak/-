...
local EnhancementUIManager = ModuleManager:GetModule("EnhancementUIManager")
...
local EnhancementNotifications = ModuleManager:GetModule("EnhancementNotifications")
local CompanionUIManager = ModuleManager:GetModule("CompanionUIManager")
local LeaderboardUIManager = ModuleManager:GetModule("LeaderboardUIManager") -- <<< 리더보드 UI 매니저 로드
...
tryInit(EnhancementUIManager, "EnhancementUIManager");
tryInit(CompanionUIManager, "CompanionUIManager")
tryInit(LeaderboardUIManager, "LeaderboardUIManager") -- <<< 리더보드 UI 매니저 초기화
...
trySetupUI(LeaderboardUIManager, "LeaderboardUIManager") -- <<< 리더보드 UI 참조 설정
...
if inventoryUpdatedEvent then
	inventoryUpdatedEvent.OnClientEvent:Connect(function()
		...
		if EnhancementUIManager and EnhancementUIManager.DisplayItemForEnhancement and enhVisible then pcall(EnhancementUIManager.DisplayItemForEnhancement, EnhancementUIManager.currentSelectedItemInventoryIndex) end
		if EnhancementUIManager and EnhancementUIManager.PopulateEnhanceableItems and enhVisible then pcall(EnhancementUIManager.PopulateEnhanceableItems) end
		...
		if LeaderboardUIManager and LeaderboardUIManager.leaderboardFrame and LeaderboardUIManager.leaderboardFrame.Visible then -- <<< 리더보드 새로고침 추가
		...
		local success, err = pcall(function()
			local function safeCall(module, funcName, ...)
				if module and module[funcName] and typeof(module[funcName]) == "function" then
					...
					if module == PurchaseNotifications or module == EnhancementNotifications or module == SkillShopUIManager or module == CompanionUIManager then
						isMethod = true
					end
			...
			print("GuiManager: Debug - Checking EnhancementUIManager. EnhancementUIManager is:", typeof(EnhancementUIManager)); local enhFrame = EnhancementUIManager and EnhancementUIManager.enhancementFrame; local isVisible = enhFrame and enhFrame.Visible; if isVisible then print("GuiManager: Debug - Calling EnhancementUIManager.DisplayItemForEnhancement because frame is visible."); safeCall(EnhancementUIManager, "DisplayItemForEnhancement", EnhancementUIManager.currentSelectedItemInventoryIndex); safeCall(EnhancementUIManager, "PopulateEnhanceableItems") end
			local compFrame = CompanionUIManager and CompanionUIManager.companionFrame; local compVisible = compFrame and compFrame.Visible
			if CompanionUIManager and compVisible then
		...
		elseif notificationType == "EnhancementResult" then
			if EnhancementNotifications and data then
				if data.success and data.itemName and data.newLevel then
					print(string.format("DEBUG: 클라이언트: EnhancementNotifications.ShowEnhancementSuccess 호출 시도 - 아이템: %s, 레벨: %d", data.itemName, data.newLevel))
					pcall(EnhancementNotifications.ShowEnhancementSuccess, EnhancementNotifications, data.itemName, data.newLevel)
					handled = true
				elseif not data.success and data.itemName and data.reason then
					print(string.format("DEBUG: 클라이언트: EnhancementNotifications.ShowEnhancementFailed 호출 시도 - 아이템: %s, 이유: %s", data.itemName, data.reason))
					pcall(EnhancementNotifications.ShowEnhancementFailed, EnhancementNotifications, data.itemName, data.reason)
					handled = true
				end
			end
