...
-- local EnhancementResultEvent = ReplicatedStorage:FindFirstChild("EnhancementResultEvent") -- 이 이벤트는 NotifyPlayerEvent로 대체될 수 있음 (주석 처리 또는 삭제 가능)
...
function InventoryManager.EquipItem(player, itemId)
	...
	if not requirementMet then
		...
		if NotifyPlayerEvent then print("[DEBUG] InventoryManager.EquipItem: Firing NotifyPlayerEvent for unmet requirement:", failMessage); NotifyPlayerEvent:FireClient(player,"EquipFailed",{reason = failMessage}); print("[DEBUG] InventoryManager.EquipItem: NotifyPlayerEvent fired.") end;
	...
	elseif itemInfo.Slot=="Accessory" then
		...
		if not targetSlot then
			...
			if NotifyPlayerEvent then NotifyPlayerEvent:FireClient(player,"EquipFailed",{reason = "모든 악세사리 슬롯이 가득 찼습니다."}) end 
			print("===== [DEBUG] InventoryManager.EquipItem 종료 (실패: 악세사리 슬롯 없음) =====");
			return false,"모든 악세사리 슬롯이 가득 찼습니다." 
		end
...
function InventoryManager.UnequipItem(player, slot, internalCall)
	...
	print("[DEBUG] UnequipItem: 현재 Equipped 상태:", HttpService:JSONEncode(playerData.Equipped)) 
	...
	if not itemAddedSuccess then
		...
		return false, addMessage or "인벤토리에 추가할 수 없습니다." 
	end
...
-- ##### 아이템 강화 함수 수정 (NotifyPlayerEvent 사용 + 장착 아이템 직접 변경 로직 제거) #####
function InventoryManager.EnhanceItem(player, inventorySlotIndex)
	print(string.format("DEBUG: EnhanceItem 호출됨 - Player: %s, Index: %s", player.Name, tostring(inventorySlotIndex))) 
	if not RunService:IsServer() then return false, "서버 전용" end
	...
	pData.Inventory = pData.Inventory or {}
	-- pData.Equipped = pData.Equipped or {} -- 더 이상 여기서 직접 사용하지 않음
	...
	if not inventorySlotIndex or not pData.Inventory[inventorySlotIndex] then
		...
		if NotifyPlayerEvent then
			print("DEBUG: 서버: 잘못된 아이템 선택 이벤트 발송 시도") 
	...
	if not itemInfo then
		if NotifyPlayerEvent then
			print("DEBUG: 서버: 알 수 없는 아이템 이벤트 발송 시도") 
	...
	if not itemInfo.Enhanceable then
		if NotifyPlayerEvent then
			print("DEBUG: 서버: 강화 불가 아이템 이벤트 발송 시도") 
	...
	if currentLevel >= maxLevel then
		if NotifyPlayerEvent then
			print("DEBUG: 서버: 최대 레벨 도달 이벤트 발송 시도") 
	...
	if not levelInfo then
		...
		if NotifyPlayerEvent then
			print("DEBUG: 서버: 더 이상 강화 불가 이벤트 발송 시도") 
	...
	if not canAfford then
		...
		if NotifyPlayerEvent then
			print(string.format("DEBUG: 서버: 강화 재료/골드 부족 이벤트 발송 시도 - 이유: %s", failReason)) 
	...
	if enhanceSuccess then
		local newLevelForInventoryItem = nextLevel
		itemSlotData.enhancementLevel = newLevelForInventoryItem -- 인벤토리 아이템 레벨 업데이트
		print("EnhanceItem: 강화 성공! Inventory Item ID:", itemId, "New Level:", newLevelForInventoryItem)

		if NotifyPlayerEvent then
			print(string.format("DEBUG: 서버: 강화 성공 이벤트 발송 시도 - 아이템: %s, 새 레벨: %d", itemInfo.Name or "아이템", newLevelForInventoryItem)) 
			NotifyPlayerEvent:FireClient(player, "EnhancementResult", {
				success = true,
				itemId = itemId, -- 클라이언트에서 아이템 정보를 다시 조회할 수 있도록 ID 전달
				itemName = itemInfo.Name or "아이템",
				newLevel = newLevelForInventoryItem
			})
		end

		-- === 수정된 부분: 장착된 아이템의 강화 레벨을 직접 변경하거나 스탯을 여기서 재계산하지 않음 ===
		-- 플레이어가 나중에 이 강화된 아이템을 장착하면 EquipItem 함수에서 스탯 재계산이 이루어짐.
		-- === 수정 끝 ===
		...
		if NotifyPlayerEvent then
			print(string.format("DEBUG: 서버: 강화 실패 이벤트 발송 시도 - 아이템: %s, 이유: %s", itemInfo.Name or "아이템", failReasonText)) 
			NotifyPlayerEvent:FireClient(player, "EnhancementResult", {
				success = false,
				itemId = itemId, 
				itemName = itemInfo.Name or "아이템",
